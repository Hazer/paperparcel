  ...
  @Test public void listOfParcelableTypesTest() throws Exception {
    ...
    JavaFileObject expectedSource =
        JavaFileObjects.forSourceString("test/Test$$Wrapper", Joiner.on('\n').join(
            ...
            "  public static final Parcelable.Creator<Test$$Wrapper> CREATOR = new Parcelable.Creator<Test$$Wrapper>() {",
            "    @Override public Test$$Wrapper createFromParcel(Parcel in) {",
            "      List<Integer> child = new MutableListAdapter<>(new IntegerAdapter()).readFromParcel(in);",
            ...
            "    new MutableListAdapter<>(new IntegerAdapter()).writeToParcel(this.data.getChild(), dest, flags);",
            "  }",
            "}"
        ));
